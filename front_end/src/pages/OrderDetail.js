...
export default function OrderDetail(props) {
  ...
  const fetchOrderDetails = async () => {
      try {
          const ordersResponse = await axios.get(`/api/orders/${props.match.params.id}`);
          setOrder(prevstate=>{
            return {...prevstate, orderInfo: ordersResponse.data}
          })

          const ordersItemsResponse = await axios.get(`/api/order-items/${props.match.params.id}`);
          setOrder(prevstate=>{
    ...
    return(
     <li key={item.order_item_id} className='list-view'>
      ...
      <h6>{item.product.name}</h6>
        <Link to={`/products/${item.product.product_id}`} className="btn-primary">See product</Link>
     </li>
   )
  })}
  </ul>
